...
//    stage.addChildAt(grass, 0);
	...
	background.graphics.beginBitmapFill(loader.getResult("background5")).drawRect(0, 0, 1000, 645);
	...
	this.magmaBmp = new createjs.Bitmap(loader.getResult("magma"));
	stage.addChild(this.magmaBmp);
}
...
StageInterface.prototype.step = function(obj){
	...
	if(this.magmaBmp){
		this.magmaBmp.setTransform(
				this.holes[0].GetPosition().x * SCALE,
				this.holes[0].GetPosition().y * SCALE,
				0.88, 0.9, this.holes[0].GetAngle()*180/Math.PI%360, 0, 0, canvasWidth/2+70 ,canvasHeight/2+22
				);
	}
}
